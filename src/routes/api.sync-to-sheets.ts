...
const formatChennaiTimestamp = (value: string | Date = new Date()) => {
...
const toMemberObject = (name: string, email: string, phone: string, department: string, college: string) => ({
  name: name || '',
  email: email || '',
  phone: phone || '',
...
const getNormalizedRegistration = (normalizedRegistration: any, registration: any) => {
...
const toSheetRow = (normalized: any) => {
    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { registration, normalizedRegistration } = body
          ...
          const appendResults: Array<{ sheetId: string; result: any }> = []
